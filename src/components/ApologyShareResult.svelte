...
<div class="max-w-xl mx-auto bg-white rounded-2xl shadow-md border border-gray-200 p-6 md:p-8 text-center">
  ...
  <p class="text-gray-500 mb-6">Share the link below — or publish it to make it public and findable on Google.</p>

  <a href={`/publish?slug=${slug}`} class="block mb-6 px-6 py-4 bg-purple-600 text-white text-lg font-semibold rounded-xl hover:bg-purple-700 transition-colors shadow-md">
    Publish this page →
  </a>

  <div class="flex items-center gap-2 mb-4">
    <input class="flex-1 px-4 py-3 border border-gray-300 rounded-xl text-sm bg-gray-50" value={shareUrl} readonly />
    <button on:click={copy} class="px-4 py-3 bg-white text-purple-700 border-2 border-purple-300 rounded-xl hover:bg-purple-50 transition-colors font-medium whitespace-nowrap">{copied ? "Copied!" : "Copy link"}</button>
  </div>
